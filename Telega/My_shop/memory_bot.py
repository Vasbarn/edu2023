    ...
    try:
        with open(path, "r", encoding="utf-8") as file:
            ...
            for record in data.values():
                try:
                    await dp.storage.set_state(user=record.get("id"), state=record.get("state"))
                    await dp.storage.set_data(user=record.get("id"), data=record)
                except ValueError:
...
async def save_memory(state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        with open(path, "w", encoding="utf-8") as file:
            ...
            data_from_file[str(data.get("id"))] = var_dict
