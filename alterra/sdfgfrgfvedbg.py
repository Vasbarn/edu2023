...
class ParserFormulaM2(Parser):
	...
	def load_catalog(self):
		...
		for _ in range(5):
			try:
				...
				groups_goods = soup.find_all("span", class_="catalog__list-item-value")
				for group_good in groups_goods:
					href = group_good.get("rel")
					if not href:
						continue
					href = self.main_url + group_good.get("rel")[1:]
	...
	def do_some(self, url: str):
		...
		data = self.data
		try:
			pages = soup.find(class_="pagination main__pagination catalog-pagination")
			if pages is None:
				max_page = 1
			else:
				max_page = int(soup.find_all(class_="pagination__link")[-1].text.strip())
			for page in range(max_page + 1):
				goods = soup.find_all("div", class_="products-grid__col products-grid__col-vertical")
				for good in goods:
					href = self.main_url + good.find(class_="product-card__title").find("a").get("href")[1:]
