...
class Parser(ABC):
    ...
    def _get_main_url(self) -> str:
        ...
        full_path = os.path.abspath(os.path.join("", "Helpers", "Список конкурентов.xlsx"))
        df = pd.read_excel(io=full_path, sheet_name="Ссылки")
        df = df[df['Наименования'] == self.name]
        if df.empty:
            raise ValueError("Конкурента {} нет в файле".format(self.name))
        url = df["Ссылка"].values[0]
        return url
