...
def main():
    ...
    if os.path.exists(path):
        ...
        os.remove(path)
        path = os.path.abspath("../Выгрузка цен Знак.xlsx")
        df.to_excel("C:\\Users\\Admin\\PycharmProjects\\edu2023\\alterra\\Выгрузка цен Знак.xlsx",
                    sheet_name="Лист 1",
                    index=False)
        df2.to_excel("C:\\Users\\Admin\\PycharmProjects\\edu2023\\alterra\\Выгрузка цен Знак.xlsx",
                    sheet_name="Лист 2",
                    index=False)
    else:
        with ExcelWriter(path, mode="w") as writer:
            df.to_excel(writer, sheet_name="Лист 1")
            df2.to_excel(writer, sheet_name="Лист 1")
    path = os.path.abspath("../Выгрузка цен Знак.xlsx")
